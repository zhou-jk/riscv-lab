...
class Lsu extends Module with HasExceptionNO {
  ...
  // 检测访存地址是否对齐
  val ex_out = WireInit(io.ex)  // 使用WireInit初始化Wire，避免直接修改输入
  val is_load = LSUOpType.isLoad(op)
  val is_store = LSUOpType.isStore(op)
  val addr_check = MuxLookup(op(1, 0), true.B)(Seq(
    "b00".U -> true.B,                         // lb/sb: 一定对齐
    "b01".U -> (mem_addr(0) === 0.U),          // lh/sh: 低位为0时对齐
    "b10".U -> (mem_addr(1, 0) === 0.U),       // lw/sw: 低两位为0时对齐
    "b11".U -> (mem_addr(2, 0) === 0.U)        // ld/sd: 低三位为0时对齐
  ))
