...
object RVCSRInstr extends HasInstrType {
  ...
  val table = Array(
    ...
    ECALL  -> List(InstrI, FuType.csr, 0.U(2.W) ## CSROpType.ecall),
    EBREAK -> List(InstrI, FuType.csr, 0.U(2.W) ## CSROpType.ebreak),
    MRET   -> List(InstrI, FuType.csr, 0.U(2.W) ## CSROpType.mret),
    WFI    -> List(InstrI, FuType.alu, 0.U(2.W) ## ALUOpType.add), // 作为nop处理
    FENCE  -> List(InstrI, FuType.alu, 0.U(2.W) ## ALUOpType.add)  // 作为nop处理
  )
}
